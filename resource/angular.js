...
var app = angular.module("modApp", []);		// the angular "app"
app.controller("modCtrl", ["$scope", "$http", function($scope, $http) {	// the angular "controller"
	...
	initialize();
	// TODO: load progress from cookies
	
	
	/*
	 *  TODO: functionality to save and progress to cookies
	 *  
			...
			if(unit.type == "question" && !unit.ignored) {
				...
				case "checkbox":
					...
					for (j in unit.choices) {
						...
						correct &= classify("checkbox", choice_id, $(choice_id).is(':checked') == choice.ans);
					}
				...
				// TODO (ongoing) add functionality for new question modes
				...
				for (j in unit.list) {
					...
					for (k in group.items) {
						...
						correct &= classify("checkbox", item_id, $(item_id).is(':checked') == item.ans);
	...
	$scope.generateCertificate = function() {
		...
		img.src = $scope.repo+"blank_certificate.png"
